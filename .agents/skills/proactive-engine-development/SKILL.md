...
- Preserve the Stage C2 media boundary: run camera and microphone providers as separately permissioned Python processes over a random private UDS (`0700` directory, `0600` socket), require healthy half-open Registry leases, and let their Go supervisor own stop, bounded kill, and join. CameraCapture authorizes only anonymous HOG/upper-body presence on one explicit device; it never authorizes a face model. MicrophoneCapture authorizes only local 16 kHz mono WebRTC VAD; it never authorizes recording, transcription, speaker models, or durable PCM. Do not silently choose devices or auto-restart failed workers without an explicit user retry.
