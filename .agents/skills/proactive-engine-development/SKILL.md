...
- Accept `UserReply` only as a canonical Observation produced by the PC input boundary while the application-owned response window is open. For the approved hands-free PC experience, any locally detected human speech inside `[opened_at, deadline)` counts as a reply; speech outside that window never becomes `UserReply`. Keep raw PCM, transcripts, reply content, VAD evidence, and detection heuristics outside the core.
