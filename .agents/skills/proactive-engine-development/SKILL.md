...
- Keep hands-free sensing privacy-local: require explicit first-use camera and microphone enablement, show a persistent capture indicator, process raw media only in bounded memory, and never record, transcribe, audit, or persist it by default. The first PC prototype detects presence without identity or face recognition, listens for voice only while a response window is open, serves its UI only on loopback, targets Ubuntu Linux, and uses local Speech Dispatcher for TTS.
