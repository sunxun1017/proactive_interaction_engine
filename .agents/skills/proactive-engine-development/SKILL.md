...
- Keep hands-free and biometric sensing privacy-local: require explicit per-capability and per-subject consent, show persistent capture and recognition indicators, process raw media only in bounded worker memory, discard enrollment media after template extraction, encrypt templates, and support revocation and deletion. Biometric capabilities default off, uncertain or conflicting identity falls back to anonymous, and identity must not authorize safety-critical actions.
