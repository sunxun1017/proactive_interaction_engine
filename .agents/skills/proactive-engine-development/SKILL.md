...
- Accept `UserReply` only when the trusted PC input boundary converts leased `SpeechActivity` inside the application-owned `[opened_at, deadline)` response window. Treat that window as the only direction signal, ignore the deprecated worker `addressing_agent` claim, normalize accepted speech confidence to 1, and keep raw PCM, transcripts, reply content, VAD evidence, and detection heuristics outside the core.
- Require every cross-process Observation publish to carry a Registry-issued lease. Validate lease existence, health, half-open expiry, source instance, declared capability, and the active scenario's explicit Provider selection before submission. Route gRPC ingress only through `Runner.SubmitObservation`, never directly through `Engine.Process`; fail closed on transport payloads not explicitly supported by the ingress slice.
...
- Compose scenarios from explicitly selected, strongly typed capabilities with validated required, optional, health, authorization, and deterministic fallback rules. Do not introduce arbitrary dynamic plugins, untyped capability maps, silent provider selection, or hot-unload compatibility before a real need exists.
- Keep hands-free and biometric sensing privacy-local: require explicit permission per sensing capability and separate per-subject consent for template-backed identification or verification, show persistent capture and recognition indicators, process raw media only in bounded worker memory, discard enrollment media after template extraction, encrypt templates, and support revocation and deletion. Biometric capabilities default off, uncertain or conflicting identity falls back to anonymous, and identity must not authorize safety-critical actions.
- Run production biometric inference only on an explicitly configured NVIDIA CUDA device with CPU fallback disabled. Pin the unified Python environment and every model artifact in Git-managed manifests, run workers offline, and treat GPU/model failure as that Provider becoming unavailable so the declared anonymous fallback, VAD, silence, templates, and P0 remain operational. Never promote model-card or example scores to product thresholds; require a versioned calibration artifact bound to the exact model digest and preprocessing protocol.
- On Ubuntu, obtain the production biometric-vault master key only from the login-session Secret Service over the encrypted DH session: never use the plain session, a filesystem key, a silent fallback, or key regeneration when finalized protected data exists. Keep lookup attributes fixed and non-PII, do not cache the key, and use only static fake keys in tests.
- Preserve the Stage C2 media boundary: run camera and microphone providers as separately permissioned Python processes over a random private UDS (`0700` directory, `0600` socket), require healthy half-open Registry leases, and let their Go supervisor own stop, bounded kill, and bounded join. Generate a fresh instance ID for every process lifetime and match runtime health on both Provider ID and current instance; never accept a stale lease from a previous child. Reconcile only the latest durably persisted permission state, and never block a successful permission response on a saturated runtime notification. CameraCapture authorizes only anonymous HOG/upper-body presence on one explicit device; it never authorizes a face model. MicrophoneCapture authorizes only local 16 kHz mono WebRTC VAD; it never authorizes recording, transcription, speaker models, or durable PCM. Do not silently choose devices or auto-restart failed workers without an explicit user retry.
- Share each physical camera or microphone through exactly one supervised device-owner process while keeping every vision/audio capability as an independently leased logical Provider. Restart the complete device group with a fresh instance and revoke all group leases whenever its effective authorized Provider set changes; pass only that exact set to the child. Send application-issued biometric work over the same private UDS as a capacity-one latest desired-state stream with no replay; revalidate only the base capture capability and capabilities used by the current task. Keep encoded templates inside the identity-control adapter and generated transport boundary, omit profile/template references from verification work, and never generalize this seam into an event bus.
