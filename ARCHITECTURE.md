...
应用向生物 Worker 下发的窗口/挑战也不使用通用消息总线。`IdentityWorkerControlService` 在现有私有 UDS 上为视觉和音频各提供一条 server stream，只保留容量为 1 的最新期望状态，重连从 idle 开始且不 replay。视觉和音频各由唯一设备 owner 进程持有物理 camera/microphone，但 Presence、FaceDetection、FaceIdentification、FaceLiveness、VAD、SpeakerIdentification、SpeakerVerification 仍是七个独立单能力 Provider/lease。Supervisor 按最新持久权限计算精确 Provider 子集；有效集合变化会停止整组、撤销全部组内 lease、重读稳定权限、生成新 instance，并只把该子集传给子进程。发布 active work 时只重验 base capture 与本次任务实际使用的能力，避免无关 optional Provider 故障连带阻塞。identification 的有界 encoded template 只存在于该 adapter/transport 边界；verification work 在结构上不携带 expected profile 或 template reference。
...
Stage C3 当前达到 model-enabled foundation checkpoint，而不是阶段完成。已实现严格的 scenario schema v2、Provider operational profile 校验、crash-safe 加密 biometric catalog/template vault、Secret Service production master-key adapter、确定性 Identity Resolver、identification/verification coordinators、五个 identity evidence RPC、opt-in desktop composition、动态 readiness seam、两条私有 identity worker-control stream、双设备进程/多逻辑 Provider Supervisor 基础，以及 pinned CUDA-only face/liveness/speaker 模型与固定模板 codec；fake/in-memory 测试覆盖应用边界，真实 artifact conformance 已验证目标 NVIDIA GPU 执行。scenario schema v2 不是 Provider protocol 或 Protobuf package v2，当前跨进程契约仍位于 `proactive.platform.v1`。

尚未实现受 supervisor 管理的真实 face/speaker/liveness Provider 执行循环、注册媒体采集、校准阈值和 UI enrollment/status。`cmd/desktop.Build` 已支持严格 opt-in identity composition，但检入配置仍保持 disabled，未显式配置和校准时不会加载密钥、模板或注册 identity RPC。canonical subject identity Observation、私有记忆门控、个性化欢迎和共享家庭组合属于后续 C4。
